...
volatile unsigned firstValue;
volatile unsigned secondValue;
unsigned difference = 0xFFFFFFFF;
unsigned min = 0xFFFFFFFF;
unsigned baud;

void TIMER2_IRQHandler(void){
	LPC_TIM2 ->IR |= (1<<5);		//Interrupt Reset war hier wichtig warum auch immer sonst zweimal Interrupt
...
int zeitdauer = 10; // zeitdauer in s angeben
int frequenz = 25000000; // Frequenz in Hz

void  init_Timer0_for_UART(void) {
	LPC_TIM0->TCR = 0x02;		//TIMER0 zurücksetzen
	LPC_TIM0->PR  = 0x00;		//TIMER0 prescaler auf 0 setzen
	LPC_TIM0->CTCR	= 0x0;       
	LPC_TIM0->MR0 =zeitdauer * frequenz;  
	LPC_TIM0->IR  = 1;		//Interrupts reset
	LPC_TIM0->MCR = 0x03;		//Interrupt bei match
	LPC_TIM0->TCR = 0x01;		//TIMER0 starten
}
