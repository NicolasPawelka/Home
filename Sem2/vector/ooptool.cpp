...
std::string argsp_t::option(const std::string& key, const std::string& defval) {
    auto it = _options.find(key);
    if (it == _options.end()) {
        return defval;
    }
...
int argsp_t::int_option(const std::string& key) {
    auto it = _options.find(key);
    if (it == _options.end()) {
...
int argsp_t::int_option(const std::string& key, int defval) {
    auto it = _options.find(key);
    if (it == _options.end()) {
        return defval;
    }
...
long argsp_t::long_option(const std::string& key) {
    auto it = _options.find(key);
    if (it == _options.end()) {
...
long argsp_t::long_option(const std::string& key, long defval) {
    auto it = _options.find(key);
    if (it == _options.end()) {
        return defval;
    }
...
double argsp_t::double_option(const std::string& key) {
    auto it = _options.find(key);
    if (it == _options.end()) {
...
double argsp_t::double_option(const std::string& key, double defval) {
    auto it = _options.find(key);
    if (it == _options.end()) {
        return defval;
    }
...
string argsp_t::pos(size_t idx, const std::string& defval) {
    if (idx >= _positionals.size()) {
        return defval;
    }
    return _positionals[idx];
...
string Timer::human_format(double secs) {
    ...
    uint64_t us = ns/1000;
    uint64_t ms = us/1000;
    return _human_format(static_cast<long>(ns), 
                         static_cast<long>(us),
                         static_cast<long>(ms));
}
