...
const uint16_t My_CAN_ID = 0x715;
    ...
    while(1)
		{
				if(AD_New_Value_Ready)
				{
					...
					if (Delta_Voltage > 0.5 || Delta_Voltage < -0.5) {
						sprintf(AD_Val_String, "%.3f", AD_Voltage);
						for (i = 0; i < My_CAN_TX_Message.length; i += 1)
							My_CAN_TX_Message.data[i] = AD_Val_String[i];
        ...
        GLCD_SetTextColor(Blue);                        /* Set the Text Color */ 	
				...
				if(CAN1_New_RX_Data_Stored)
				{
					...
					for (i = 0; i < My_CAN1_RX_message.length; i += 1)
						AD_Val_String[i] = My_CAN1_RX_message.data[i];
		  		    /*************************************************/
               ...
               i = 5; //Select GLCD lines 5 and 6 for output						
					...
					GLCD_DisplayString(i, 13, ID_String);/* print string to LCD */
					GLCD_DisplayString(i+1, 14, AD_Val_String);/* print string to LCD */		
          CAN1_New_RX_Data_Stored = 0;					
				...
				if ( My_CAN2_RX_message.id != GLCD_Last_Shown_ID){
					...
					sprintf(ID_String, "%x", My_CAN2_RX_message.id);
					for (i = 0; i < My_CAN2_RX_message.length; i += 1)
						AD_Val_String[i] = My_CAN2_RX_message.data[i];
		  		    /*************************************************/
					//Put some code lines here
					//Goal: - Convert the received ID value to a string called ID_String
					//      - Collect the received and convert them to the string AD_Val_String
