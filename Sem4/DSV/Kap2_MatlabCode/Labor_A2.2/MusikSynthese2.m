...
% Mit Oberschwingungen für "Oboenklang" * feldes * 13/09/2006
A = 220; 	% pitch in Hz
Dh = A*2^(5/12); 	% high
C = A*2^(3/12);
B = A*2^(2/12);
...
for k=1:length(pitch)	% for loop
  ...
  for kk=1:length(harmonics)	% inner for loop
    s = s + 0.1*harmonics(kk)*sin(kk*w*n);   % sinusoidal tone
  end
...
FIG1 = figure('Name','Musiksynthese: Mit Obertönen',  'NumberTitle', 'off');
plot(t,music)
